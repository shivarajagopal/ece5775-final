...
void run_voice_recognition(void)
{
  ...
  float audio_sample;
  float audio_samples[16000]; //reproduction of the samples that are sent to the
  u32 classification;         //voice rec for debugging
  ...
  while (1)
  {
	...
	for(sampleNum = 0; sampleNum < 96000; sampleNum ++) {
		...
		if(sampleNum%6 == 0) //only use every 6th sample
		{
			...
			audio_sample = ( (float)u32Data ) / MAX_AMPLITUDE;
