...
entity voicerec_voicerec_io_s_axi is
    generic (
...
end entity voicerec_voicerec_io_s_axi;
...
architecture behave of voicerec_voicerec_io_s_axi is
    ...
    constant ADDR_NEW_SAMPLE_DATA_0 : INTEGER :=16#18#;
    constant ADDR_NEW_SAMPLE_CTRL : INTEGER :=16#1c#;
    ...
    signal int_new_sample      : UNSIGNED(31 downto 0);
...
begin
    ...
    begin
        if (ACLK'event and ACLK = '1') and ACLK_EN = '1' then
            if (ar_hs = '1') then
                ...
                when ADDR_NEW_SAMPLE_DATA_0 =>
                    rdata_data <= RESIZE(int_new_sample(31 downto 0), 32);
                when others =>
                    rdata_data <= (others => '0');
                end case;
            end if;
    ...
    new_sample <= STD_LOGIC_VECTOR(int_new_sample);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NEW_SAMPLE_DATA_0) then
                    int_new_sample(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_new_sample(31 downto 0));
                end if;
            end if;
        end if;
    end process;
